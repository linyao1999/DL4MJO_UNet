...
#         # Generate MCDO masks
#         # mcdo_masks_conv1 = torch.bernoulli(torch.ones(1, num_filters_enc, dimx, dimy) * (1 - 0.1)).cuda() / 0.9
#         # mcdo_masks_conv2 = torch.bernoulli(torch.ones(1, num_filters_enc, dimx, dimy) * (1 - 0.1)).cuda() / 0.9
#         # mcdo_masks_conv3 = torch.bernoulli(torch.ones(1, num_filters_enc, dimx, dimy) * (1 - 0.1)).cuda() / 0.9
#         # mcdo_masks_conv4 = torch.bernoulli(torch.ones(1, num_filters_enc, dimx, dimy) * (1 - 0.1)).cuda() / 0.9
#         # mcdo_masks_conv5 = torch.bernoulli(torch.ones(1, num_filters_enc, dimx, dimy) * (1 - 0.1)).cuda() / 0.9
#         # mcdo_masks_conv6 = torch.bernoulli(torch.ones(1, num_filters_dec1, dimx, dimy) * (1 - 0.1)).cuda() / 0.9
#         mcdo_masks_outline1 = torch.bernoulli(torch.ones(nhidden1) * (1 - 0.2)).cuda() / 0.8
#         mcdo_masks_outline2 = torch.bernoulli(torch.ones(nhidden2) * (1 - 0.2)).cuda() / 0.8
#         mcdo_masks_outline3 = torch.bernoulli(torch.ones(nhidden3) * (1 - 0.2)).cuda() / 0.8
#         # print(mcdo_masks_outline2)

#         for batch in testing_data_loader:
#                 batch = batch.cuda()
#                 net.eval()
#                 preds.append(net(batch).data.cpu().numpy())

#         autoreg_pred_dis[:,k,:] = np.concatenate(preds, axis=0)
...
#         ),
#         attrs=dict(
#                 description=mjo_ind+' prediction distribution'
#         ),
#         name=mjo_ind+'p_dis',
# )
