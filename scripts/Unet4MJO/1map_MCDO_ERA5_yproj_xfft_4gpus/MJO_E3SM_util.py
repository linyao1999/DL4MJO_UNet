...
def wk_plot_sym(sym, tlt='', logflg=True, savflg=False, pltDispCurve=True, fb=[0,0.48], center0=False, filename='wk.png', vmin=-3.2, vmax=2.4):
    ...
    if logflg:
        if center0:
            ...
            c = ax.contourf(wavenumber, frequency, np.log10(sym.T), cmap=cmap, levels=np.linspace(vmin,vmax,12), vmin=vmin, vmax=vmax)
    else:
