...
# vn = ['u200','u850','olr','tcwv','v200','T200','u500','v500','v850','Z200','Z500','Z850','T500','T850','q200','q500','q850','sst']
...
dataflg = os.environ["dataflg"]  # new or new40
...
if mjo_ind=='RMM':
    Fnmjo = '/pscratch/sd/l/linyaoly/ML_MJO_2024_redo/data/rmm/RMM_ERA5_daily_1979to2012.nc'
elif mjo_ind=='ROMI':
    Fnmjo = '/pscratch/sd/l/linyaoly/ML_MJO_2024_redo/data/rmm/ROMI_ERA5_daily_1979to2014.nc'

# NOTE: COMMENT STARTS

data_save = '/pscratch/sd/l/linyaoly/ML_MJO_2024_redo/output/19maps_MCDO_ERA5_yproj_xfft_4gpus_new/'
...
for levelnloop in np.arange(0,nmem*nmaps):
...
for leveln in np.arange(0,nmem*nmaps):
...
# # NOTE: COMMENT STARTS
net.eval()
torch.save(net.state_dict(), model_save+'predicted_MCDO_UNET_'+str(len(vn))+'maps'+str(lat_lim)+'deg_'+mjo_ind+'_ERA5_lead'+str(leadmjo)+'_dailyinput_m'+str(m)+mflg+'_wnx'+str(wnx)+wnxflg+'_c'+str(c)+'_nmem'+str(nmem)+dataflg+'.pt')
# # NOTE: COMMENT ENDS
...
ds.to_netcdf(path_forecasts+'predicted_MCDO_UNET_'+str(len(vn))+'maps'+str(lat_lim)+'deg_'+mjo_ind+'ERA5_'+str(m)+'modes'+mflg+'_wnx'+str(wnx)+wnxflg+'_lead'+str(leadmjo)+'_dailyinput_'+str(ysta)+'to'+str(yend)+'_c'+str(c)+'_mem'+str(nmem)+'d'+dataflg+'.nc', mode='w')
