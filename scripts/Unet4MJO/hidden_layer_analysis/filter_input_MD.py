...
exp_num = str(os.environ['exp_num'])  # experiment number

# below are parameters for filtering the input 
cut_m = int(os.environ['cut_m'])  # number of meridional modes
cut_k = int(os.environ['cut_k'])  # number of zonal modes
cut_m_flg = os.environ['cut_m_flg']  # flag of cut_m
cut_k_flg = os.environ['cut_k_flg']  # flag of cut_k

print(f'{mjo_ind} {leadmjo} {exp_num} {cut_m} {cut_k} {cut_m_flg} {cut_k_flg}')

testystat = 2015  # validation starts
dataflg = 'new'
nmem = int(1)  # the number of how many days we want to include into the input maps

testyend =  2022 # str(np.datetime64('2019-12-31'))- np.timedelta64(leadmjo+nmem,'D'))  # validation ends
...
datadir = '/pscratch/sd/l/linyaoly/ERA5/reanalysis/fltano120/new2024/'
...
path_forecasts = '/pscratch/sd/l/linyaoly/ERA5/Unet4MJO/1map_MCDO_ERA5_yproj_xfft_4gpus_new/distillation/'
model_save = f'/pscratch/sd/l/linyaoly/ERA5/Unet4MJO/1map_MCDO_ERA5_yproj_xfft_4gpus_new/modelsave{exp_num}/'
...
def projection(vn, c=51, m=1, mflg='off', wnx=10, wnxflg='all', time_range=['1979-01-01','2019-12-31'], lat_range=[90, -90], pic_save='./',dataflg='new'):
    ...
    if dataflg=='raw':
        ...
        fn = '/pscratch/sd/l/linyaoly/ERA5/reanalysis/fltano120/new2024/'+vn+'.fltano120.1979to2024based1979to2012.nc'
    elif dataflg=='new40':
...
psi_test_input_Tr_torch, psi_test_label_Tr_torch, psi_test_label_Tr, M = load_test_data(vn,Fnmjo,leadmjo,mem_list,testystat,testyend,c,cut_m,cut_m_flg,cut_k,cut_k_flg,lat_lim,mjo_ind,pic_save,dataflg=dataflg)
...
net.load_state_dict(torch.load(model_save+'predicted_MCDO_UNET_olr20deg_'+mjo_ind+'_ERA5_lead'+str(leadmjo)+'_dailyinput_m'+str(m)+mflg+'_wnx'+str(wnx)+wnxflg+'_c'+str(c)+'_nmem'+str(nmem)+'.pt'))
net.eval()
...
ds.to_netcdf(path_forecasts+'flt_input_predicted_MCDO_UNET_'+vn+str(lat_lim)+'deg_'+mjo_ind+'ERA5_'+str(m)+'modes'+mflg+'_wnx'+str(wnx)+wnxflg+'_lead'+str(leadmjo)+dataflg+'.nc', mode='w')
