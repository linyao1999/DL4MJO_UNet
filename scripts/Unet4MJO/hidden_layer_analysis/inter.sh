...
generate_commands() {
    for lead0 in 15; do
        ...
        for exp0 in 1; do
            export exp=$exp0
            for channel0 in {130..133}; do
                export channel=$channel0
                logname=rmm_lead$lead\_channel$channel\_exp$exp
                echo "CUDA_VISIBLE_DEVICES=$(( ($exp - 1) / 4 )) lead=$lead channel=$channel exp=$exp python3 contribution_rmm.py > /pscratch/sd/l/linyaoly/outlog/$logname.txt"
            done
        done
    done
}
