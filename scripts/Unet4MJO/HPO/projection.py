...
def projection(vn, c=51, m=1, mflg='off', wnx=1, wnxflg='off', time_range=['1979-01-01','2019-12-31'], lat_range=[90, -90], pic_save='./'):
    ...
    if pic_save != 'None':
        fig_file_path = pic_save+vn+str(lat_range[0])+'deg_m'+str(m)+mflg+'_wnx'+str(wnx)+wnxflg+'.jpg'
        if not os.path.exists(fig_file_path):  # create a snapshot of the input map if the file does not exist. 
            ...
            im = ax[0].contourf(lon, lat.values, olr_re_fft[0,:,:], cmap='Blues_r')
            ...
            im = ax[1].contourf(lon, lat.values, tmp, cmap='Blues_r')
